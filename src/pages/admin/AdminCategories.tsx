...
const AdminCategories = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    else { toast({ title: editingId ? "Categoría actualizada" : "Categoría creada" }); setName(""); setEditingId(null); load(); }
    setSaving(false);
  };

  const handleEdit = (item: Category) => { setEditingId(item.id); setName(item.name); };

  const handleDelete = async (id: string) => {
    if (!confirm("¿Eliminar esta categoría?")) return;
    const { error } = await supabase.from("categories").delete().eq("id", id);
    if (error) toast({ title: "Error", description: error.message, variant: "destructive" });
    else { toast({ title: "Categoría eliminada" }); load(); }
  };
    ...
    <div className="max-w-2xl">
      ...
      <form onSubmit={handleSubmit} className="flex gap-3 mb-8">
        <div className="flex-1 space-y-1">
          <Label htmlFor="cat-name" className="sr-only">Nombre</Label>
          <Input id="cat-name" placeholder="Nombre de la categoría" value={name} onChange={(e) => setName(e.target.value)} required maxLength={100} />
        </div>
